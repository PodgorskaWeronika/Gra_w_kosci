...
class Dice(object):
    ...
    # sprawdza czy jest cokolwiek (wszystkie różne)
    ...
    def score_report(self, lst):
        ...
        print('[2] Dwójka    ', self.check_number(2, lst), '\t\t-> ', self.quantity_numbers(2, lst))
        print('[3] Trójka    ', self.check_number(3, lst), '\t\t-> ', self.quantity_numbers(3, lst))
        print('[4] Czwórka   ', self.check_number(4, lst), '\t\t-> ', self.quantity_numbers(4, lst))
        print('[5] Piątka    ', self.check_number(5, lst), '\t\t-> ', self.quantity_numbers(5, lst))
        print('[6] Szóstka   ', self.check_number(6, lst), '\t\t-> ', self.quantity_numbers(6, lst))
        print('')
    ...
    def position_to_score(self, poz_char: str, lst):
        if poz_char.isdigit():
            return self.quantity_numbers(int(poz_char), lst)
        mapping = {
            'A': lambda lst: self.max_sum_one_pair_list(lst),
            'B': lambda lst: self.sum_two_pairs(lst),
            'C': lambda lst: self.sum_small_straight(lst),
            'D': lambda lst: self.sum_big_straight(lst),
            'E': lambda lst: self.sum_three(lst),
            'F': lambda lst: self.sum_full(lst),
            'G': lambda lst: self.sum_carriage(lst),
            'H': lambda lst: self.sum_poker(lst),
            'I': lambda lst: self.sum_all_elements(lst)
        }
        return mapping[poz_char.upper()](lst)
